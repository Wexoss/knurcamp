...
function zakupy(balansPortfela, towary) {
  ...
  let a = 2;
  let najdrozszyTowar = { nazwa: "Knur", koszt: 0, liczbaSztuk: 2137 };
  while (a > 1) {
    for (const towar of towary) {
      // sprawdza każdy towar po kolei, jeśli jakiś jest droższy to go nadpisuje
      if (
        towar.koszt > najdrozszyTowar.koszt &&
        balansPortfela >= towar.koszt &&
        towar.liczbaSztuk >= 1
      ) {
        najdrozszyTowar = towar;
      }
    }

    for (const towar of towary) {
      if (towar.nazwa === najdrozszyTowar.nazwa) {
        // usuwa sztuke towaru po zakupie
        towar.liczbaSztuk--;
      }
    }
    ...
    najdrozszyTowar = { nazwa: "Knur", koszt: 0, liczbaSztuk: 69 };
  }
  // MIEJSCE NA TWÓJ KOD
