...
*/
// Miejsce na Twój kod.
const pociong = (tablice) => {
  return tablice.reverse();
};
/* 3 */
/*
...
*/
// Miejsce na Twój kod.
const inzynieriaOdtylcowa = (stringinienadupie) => {
  return stringinienadupie.split("").reverse().join("");
};
/* 6 */
/*
...
// Miejsce na Twój kod.
const pascal = (obiekt) => {
  return String(Object.values(obiekt));
};
/* 7 */
/*
...
// Miejsce na Twój kod.
const xNzq = (tablica) => {
  const posegregowane = tablica.sort((a, b) => a.strona - b.strona);
  const ksiegaxNzq = [];

  for (let i = 0; i < posegregowane.length; i += 2) {
    ksiegaxNzq.push(posegregowane[i]);
  }
  return ksiegaxNzq;
};
/* 8 */
...
*/
// Miejsce na Twój kod.
const detektor = (ciag, wzur) => {
  let coMaSzukac = new RegExp(wzur, "g");
  let ileRazy = ciag.match(coMaSzukac);
  if (ileRazy === null) {
    return 0;
  } else {
    return ileRazy.length;
  }
};
...
const mors = (tajnawiadomosc) => {
  ...
  const podzielony = tajnawiadomosc.split(" ");
  let ukrytaWiadomosc = "";

  for (const podzialka of podzielony) {
    ukrytaWiadomosc += morswinCode[podzialka];
  }

  return ukrytaWiadomosc;
};
